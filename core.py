...
Versão revisada — correções aplicadas:
  - calcular_t2: LSC via distribuição F (fase I) em vez de chi2
  - extrair_vars_formula: removido '%' do regex para compatibilidade com df.eval()
  - calcular_indicador: validação de df_eval vazio / todo-NaN antes do eval
  - gerar_dataframe_limpo: conversão vetorizada antes do loop de limites
  - construir_limites_cfg: função utilitária exportada para evitar duplicação na UI
  - limpar_serie: mantida e documentada como motor estatístico completo (Módulo 2 avançado)
"""
...
def construir_limites_cfg(limites_customizados: list, limites_texto: str = "") -> dict:
    """
    Consolida regras de limite em um único dict {TAG_UPPER: (min, max)}.

    Exportado para evitar duplicação na UI (Módulo 2).
    limites_customizados: lista de dicts {'tags': [...], 'minimo': x, 'maximo': y}
    limites_texto: formato legado 'TAG [min,max]'
    """
    cfg = parsear_limites_por_variavel(limites_texto)
    for regra in limites_customizados:
        for tag in regra.get("tags", []):
            cfg[tag.upper()] = (regra.get("minimo"), regra.get("maximo"))
    return cfg
...
def limpar_serie(
    ...
    """
    Motor de limpeza estatística completo (IQR, Z-score, rolling, etc.).

    Disponível para uso futuro no Módulo 2 avançado.
    Atualmente o Módulo 2 usa gerar_dataframe_limpo() (só limites de engenharia).
    """
...
def gerar_dataframe_limpo(
    ...
    Melhoria: conversão para numérico vetorizada fora do loop (mais rápido em bases grandes).
    """
    limites_cfg = construir_limites_cfg(limites_customizados, limites_texto)

    # Conversão vetorizada — mais eficiente do que converter coluna a coluna no loop
    df_num = df_original[tags_validas].apply(pd.to_numeric, errors="coerce")
...
def extrair_vars_formula(formula: str) -> list:
    ...
    Correção: removido '%?' do regex — nomes com '%' são inválidos em df.eval()
    e causavam falhas silenciosas com fórmulas como 'EFICIENCIA%'.
    """
...
def calcular_indicador(
    ...
    Melhoria: validação de df_eval vazio ou todo-NaN antes de chamar eval().
    """
    ...
    for k in range(chunk):
        ...
        # Validação: sem dados válidos no período selecionado
        if df_eval.empty or df_eval.isna().all().all():
            return None, "Dados insuficientes no período selecionado para uma ou mais variáveis."
...
def calcular_t2(df: pd.DataFrame, cols: list, nivel_confianca: float) -> tuple:
    ...
    Correção: LSC agora usa distribuição F (fase I — parâmetros estimados dos dados),
    que é estatisticamente correto quando os parâmetros populacionais são desconhecidos.

    Limite fase I (Jackson & Mudholkar, 1979):
        LSC = p(n-1)(n+1) / [n(n-p)] * F_alpha(p, n-p)

    O limite chi2 era válido apenas para fase II (parâmetros conhecidos a priori),
    o que subestimava o LSC e gerava excesso de alarmes falsos com n moderado.
    """
    ...
    if n <= p:
        raise ValueError(
            f"Número de observações ({n}) deve ser maior que o número de variáveis ({p}). "
            "Reduza as tags selecionadas ou amplie o período de análise."
        )
    ...
    # LSC fase I — distribuição F
    F_crit = f_dist.ppf(nivel_confianca, p, n - p)
    lsc_t2 = (p * (n - 1) * (n + 1)) / (n * (n - p)) * F_crit
