...
def limpar_dados(df, remover_zeros, remover_negativos, outlier_metodo, fator):
    ...
    # 1. Força a conversão para número. O que for texto de erro (ex: "Parado") vira NaN (vazio)
    df_res = df_res.apply(pd.to_numeric, errors='coerce')
    
    # 2. Agora podemos fazer a matemática tranquilamente
    if remover_zeros:
        df_res = df_res.mask(df_res.abs() < 1e-5)
    ...
    # 4. Preenchimento inteligente (Forward e Backward Fill) para tampar os buracos dos erros
