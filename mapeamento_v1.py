...
# ==========================================
    ...
    # Função Callback para gerenciar os botões do Uploader
    def acao_carregar_indicadores(apagar_tags=False):
        n = len(st.session_state.get('_lista_importada', []))
    ...
    if st.session_state.get('_lista_importada'):
        ...
        with col_load1:
            st.button(
                f"✅ Carregar {n} indicadores (preserva tags)", 
                key="btn_precarregar", 
                on_click=acao_carregar_indicadores, 
                kwargs={'apagar_tags': False}
            )
        with col_load2:
            st.button(
                f"🔄 Recarregar do zero (apaga tags)", 
                key="btn_recarregar", 
                on_click=acao_carregar_indicadores, 
                kwargs={'apagar_tags': True}
            )
