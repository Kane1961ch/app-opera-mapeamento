...
Versão revisada — correções aplicadas:
  - Fluxo B: try/except ao redor de preparar_base_pi()
  - Auditoria: acumulação correta de lixo_para_remover (não sobrescreve)
  - Auditoria: hash de coluna para detecção de duplicatas (evita df.T pesado)
  - Módulo 2: usa construir_limites_cfg() do core.py (sem duplicação)
  - Módulo 3: usa extrair_vars_formula() do core.py (sem duplicação)
  - Módulo 3: navegação automática transformada em sugestão (não force-navega)
  - Módulo 4: sharey=False para evitar distorção multi-unidade
  - Módulo 4: except estreitado para TypeError/AttributeError no filtro de data
  - Módulo 4: botão de exportação de métricas CEP (.csv)
  - Módulo 5: LSC do T² agora usa distribuição F (via core.py corrigido)
  - Módulo 5: validação n > p antes de calcular T²
  - Exportação/importação de mapeamento em JSON (persistência entre sessões)
  - Importação duplicada de io/pickle removida
"""
...
if menu == "📂 1. Carga e Auditoria":
    ...
    with st.expander("🔄 Processar Nova Base Bruta do PI AF (.xlsx → .pkl)", expanded=False):
        ...
        if arq_xlsx:
            # FIX: try/except ao redor de preparar_base_pi para evitar crash exposto
    ...
    if st.session_state['df_pi'] is None:
        ...
        if executar:
            ...
            # FIX: usa hash por coluna em vez de df.T.duplicated() — muito mais eficiente
            st.markdown("**2️⃣ Dados Clonados (Histórico 100% Idêntico)**")
            try:
                col_hashes = {
                    col: hash(df_audit[col].values.tobytes())
                    for col in df_audit.columns
                }
                hash_para_cols: dict = {}
                tags_clonadas = []
                for col, h in col_hashes.items():
                    if h in hash_para_cols:
                        tags_clonadas.append(col)
                    else:
                        hash_para_cols[h] = col
                ...
                if tags_clonadas:
                    ...
                    for clone in tags_clonadas:
                        h = col_hashes[clone]
                        original = hash_para_cols.get(h, "—")
                        rows_clone.append({"Original (manter)": original, "Clone (remover)": clone})
                    ...
                    if st.button("🗑️ Marcar clones para remoção", key="btn_marcar_clones"):
                        # FIX: acumula ao invés de sobrescrever
                ...
                st.warning(f"⚠️ Não foi possível verificar clones: {e}")
            ...
            with st.spinner("Calculando matriz de correlação..."):
                ...
                if not df_num.empty:
                    ...
                    if pares:
                        ...
                        # FIX: acumula ao invés de sobrescrever completamente
                        st.session_state['lixo_para_remover'] = list(
                            set(st.session_state.get('lixo_para_remover', []) + [p[1] for p in pares])
                        )
                    else:
...
# ==========================================
    ...
    if aplicar:
        with st.spinner("Aplicando filtros de engenharia..."):
            ...
            # FIX: usa construir_limites_cfg do core.py (sem duplicação de lógica)
            limites_cfg_ui = construir_limites_cfg(
                st.session_state.get('limites_customizados', []),
                st.session_state.get('limites_texto', "")
            )
...
# ==========================================
        ...
        if arquivo_ind:
            ...
            if st.session_state.get('_arquivo_lido') != conteudo_hash:
                try:
                    ...
                    st.session_state['_arquivo_lido'] = conteudo_hash
    ...
    # Normaliza dados antigos no mapeamento
    ...
    # ---- Persistência do mapeamento (exportar / importar JSON) ----
    st.subheader("💾 Persistência do Mapeamento")
    col_exp, col_imp = st.columns(2)

    with col_exp:
        if st.session_state['mapeamento']:
            mapeamento_json = json.dumps(st.session_state['mapeamento'], ensure_ascii=False, indent=2)
            st.download_button(
                label="⬇️ Exportar mapeamento (.json)",
                data=mapeamento_json,
                file_name="opera_mapeamento.json",
                mime="application/json",
                help="Salve para não perder as vinculações ao recarregar a página."
            )
        else:
            st.caption("Nenhum mapeamento para exportar.")

    with col_imp:
        arq_json = st.file_uploader("📥 Importar mapeamento (.json)", type=["json"], key="uploader_mapeamento")
        if arq_json:
            try:
                mapeamento_importado = json.loads(arq_json.read().decode("utf-8"))
                if st.button("▶️ Carregar mapeamento importado", key="btn_importar_mapeamento"):
                    st.session_state['mapeamento'] = mapeamento_importado
                    st.success(f"✅ {len(mapeamento_importado)} indicador(es) importado(s)!")
                    st.rerun()
            except Exception as e:
    ...
    # FIX: usa extrair_vars_formula do core.py (elimina duplicação da lógica)
    vars_formula = extrair_vars_formula(formula_edit) or ["VAR_UNICA"]
    ...
    if salvar:
        ...
        if sem_tag:
            ...
            # FIX: navega para próximo pendente apenas como sugestão, não forçando
            pendentes = [n for n, v in st.session_state['mapeamento'].items()
                         if not v.get('var_tags') or any(not vt for vt in v['var_tags'].values())]
            pendentes_outros = [p for p in pendentes if p != nome_edit]
            if pendentes_outros:
                st.info(f"💡 Próximo pendente: **{pendentes_outros[0]}** — selecione acima para continuar.")
            st.rerun()
...
# ==========================================
    ...
    try:
        ...
        # FIX: except estreitado — apenas erros esperados do índice não-datetime
        st.caption("Índice não é datetime; filtro de data ignorado.")

    st.divider()

    # ---- Loop por indicador ----
    todas_metricas = []  # acumula para exportação
    ...
    for nome_ind, item in st.session_state['mapeamento'].items():
        ...
        with st.expander(titulo_exp, expanded=True):
            ...
            # FIX: sharey=False — evita distorção quando unidades têm escalas diferentes
            ...
            for i, (maq, serie) in enumerate(resultados.items()):
                ...
                lsc = media + 3 * desvio
                lic = media - 3 * desvio
                fora = serie[(serie > lsc) | (serie < lic)] if desvio > 0 else pd.Series(dtype=float)
                pct_fora = len(fora) / len(serie) * 100 if len(serie) > 0 else 0.0
            ...
            if metricas_globais:
                ...
                todas_metricas.extend(metricas_globais)

    # FIX: exportação consolidada de todas as métricas CEP
    if todas_metricas:
        st.divider()
        df_export = pd.DataFrame(todas_metricas)
        csv_bytes = df_export.to_csv(index=False, sep=";", decimal=",").encode("utf-8-sig")
        st.download_button(
            label="⬇️ Exportar métricas CEP (.csv)",
            data=csv_bytes,
            file_name="opera_metricas_cep.csv",
            mime="text/csv",
            help="Exporta média, desvio, LSC/LIC e % fora de controle de todos os indicadores."
        )
...
# ==========================================
    ...
    with tab2:
        ...
        st.caption("ℹ️ LSC calculado via distribuição F (fase I — parâmetros estimados dos dados), estatisticamente correto para monitoramento com histórico próprio.")
            ...
            if not cols_t2_sel:
                ...
                if t2_valores is not None:
                    ...
                    ax2.axhline(lsc_t2, color='red', linewidth=2,
                                label=f'LSC Hotelling F({confianca * 100:.1f}%): {lsc_t2:.2f}')
                    falhas_t2 = t2_serie[t2_serie > lsc_t2]
                    ax2.scatter(falhas_t2.index, falhas_t2.values, color='red', s=20, zorder=5)
                    ax2.set_title(f"T² de Hotelling — {p} variáveis  |  n={n}", fontweight='bold')
                    ax2.set_ylabel("T²")
